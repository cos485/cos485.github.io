...
Here is the Google Drive link for the homework [MNIST-Intro.ipynb](https://google.com) and [DeltaRule.ipynb](https://google.com). (Temprorary links for now) Direct yourself to the link, sign in with your Princeton account (gmail will also work). Select `Open with` > `Connected more apps`. Choose `Colaboratory` > `Connect`. Now select `Open with Colaboratory` and press `Open in Playground`. Make sure to save your work in progress by pressing `Copy to Drive` or `File` > `Save a copy in Drive`.

[Google Colaboratory](`https://colab.research.google.com/notebooks/`).

If you are using your own computer, download the sample code and start the Jupyter notebook using the commands in the same folder.
...
Either of these methods should open the Jupyter Notebook Dashboard in your browser.   
